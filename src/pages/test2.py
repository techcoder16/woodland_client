        ...
        if cdr['orig_sub'] is not None and cdr['orig_sub'] in metrics['total_time']:
            ...
            if cdr['term_sub'] is not None and cdr['term_sub'] in metrics['total_time']:
                ...
                if cdr['type'] == str(0) or cdr['type'] == '0':
                    ...
                    #     'Direction': 'Incoming'
                    # }
                    # metrics['data_calls'].append(data_outbound)
...
if __name__ == "__main__":
    try:
        ...
        start_date = datetime(year=2025, month=1, day=22, hour=23, minute=0, second=0).strftime('%Y-%m-%d %H:%M:%S')

        end_date = datetime(year=2025, month=1, day=23, hour=23, minute=0, second=0).strftime('%Y-%m-%d %H:%M:%S')
